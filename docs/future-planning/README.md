...
**Three pieces of `daemon/` are built — M0, the skeleton (PR #340); M5a, daemon-owned ptys
(PR #341); and mail in benchd (PR #342). Everything else in this directory remains unbuilt,
and nothing in it authorises building.**
...
- `bench-architecture.md` — the target shape the operator approved on 2026-09-25: seven
  primitives, the focus rule, drawers and rules files, who owns what, and the terminal stack.
  Read it before the roadmap.
- `bench-roadmap.md` — the milestone sequence M0–M7 and its running order, written in the
  imperative for a session that has been sent here deliberately.
...
documents are ahead of it, and the failure mode is an agent helpfully making them true.
