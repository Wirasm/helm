 ...
 * reach a pi extension, so the rule is written twice and `hooks/mailbox-conformance.mjs`
 * executes both against the Swift it is copied from. Swift's four textual guards are all here —
 * unset or blank, the canonical domain, the legacy domain, and a name carrying `/` — and the
 * fifth is not, because it cannot be: `UserDefaults(suiteName:) != nil` is a framework call, and
 * its one documented refusal beyond the two domains above is `NSGlobalDomain`.
 *
 ...
 *
 * Three rules, and they are `SpoolDirectory.resolve`'s to the letter, one directory over: an
 * explicit `HELM_MAIL_DIR` wins, else an isolated instance gets `~/.helm/mail-<suite>`, else
 * `~/.helm/mail`.
 */
