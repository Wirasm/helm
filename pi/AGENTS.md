...
**If you touched `extensions/bench/`, also run the daemon gate** (`bash daemon/test.sh`). The
extension is a sensor: it pipes each pi event into `bench hook pi` and never holds a mailbox of its
own, so the rules it depends on — who is claimed, what a hand-out carries, when a wake is allowed —
live in `daemon/crates/benchd/src/hook.rs` and are tested there.

**It is hermetic, and it proves that rather than claiming it.** Two harnesses start a real pi,
which loads a real extension, which reports to benchd — and a gate run inside a helm pane is
exactly the session benchd gives an address. Its predecessor, helm-mail, claimed mailboxes in the
operator's own `~/.helm/mail`, where live agents addressed each other (#133). Every root the
extensions honour is redirected to a per-run temp directory once, before any harness runs, and the
run then fails loudly if the operator's real bench record names one of the gate's sessions.
**Adding a harness needs no thought about this; adding a root override does** — see the sandbox
block in `scripts/test.sh` and `references/testing.md`.
