// Close a pane an agent opened — issue #176.
//
//   swift helm-close.swift <terminal-uuid>
//   swift helm-close.swift <terminal-uuid> --force     it is running something, and I mean it
...
//
// The uuid is a spawn result's `terminalId`, or the `HELM_PANE` of the pane you are running in.
// Those are the two ways to know one, and knowing it is the scoping: helm does not check that
// it spawned the pane for you (`CloseRequest` argues why at length), so an agent may tidy up a
// teammate — but only one it was told about.
//
// helm REFUSES two things, loudly, and they are not the same refusal:
//   - the pane the operator is working in. `--force` does not override this one. Their eyes are
//     not something a file on disk gets a say in.
//   - a pane with a live process in it, unless you pass `--force`. Closing it kills whatever
//     was running and loses whatever it had not written down.
//
...
let usage = """
    usage: helm-close.swift <terminal-uuid> [--force]
                            [--id NAME] [--spool DIR] [--timeout SECONDS]

    Asks the running helm to close a pane. The uuid is a spawn result's `terminalId`, or
    the `HELM_PANE` of the pane you are in. No display, no keystrokes.

    --force  close it even though something is running in it. Does NOT override helm's
             refusal to close the pane the operator is working in.
...
while let flag = arguments.first {
    ...
    default:
        ...
        guard terminal == nil else { die("one pane at a time, got \(flag) as well", .usage) }
        terminal = flag
    }
}

guard let terminal, !terminal.isEmpty else { die(usage, .usage) }
...
guard UUID(uuidString: terminal) != nil else {
    die(
        "\"\(terminal)\" is not a pane id — it is a spawn result's `terminalId`, or the "
            + "`HELM_PANE` of the pane you are running in", .usage)
}
...
let payload: [String: Any] = ["id": id, "kind": "close", "terminal": terminal, "force": force]
        ...
        let status = json["status"] as? String
    {
        ...
        case "closed":
            ...
            FileHandle.standardError.write(
                Data("helm-close: pane \(terminal) is gone\(killed)\n".utf8))
            exit(Exit.ok.rawValue)
