...
helm injects its own annotation layer into every `.html` artifact, and its listener captures — so a
page with a drawing surface, a drag-to-pan map, a game board or anything else the *pointer* means
something to is in a fight it did not know about. With the **text** tool held, a drag that selects
no text posts a dismissal, which closes the operator's notes drawer on every gesture, silently.

The operator's **default** tool is `read`, which interprets nothing at all — so that does not fire
until they pick the text tool up. **That is not a substitute for the attribute below.** It is a mode
...
no mark, no dismissal, and no anchor resolved out of it:
...
unchanged, so anything you want the operator to be able to mark and comment on goes **outside**
