...
files. The bench extension reports to benchd at `session_start`, and a gate run inside a helm pane
is exactly the session benchd gives an address. The old helm-mail extension did the same in
...
A sandbox nobody checks is a claim rather than a property, so a post-run sweep reads the operator's
real `sessions/hosted.json` for the gate's temp-dir prefix. Every session the gate can start runs
in a temp directory named with that prefix, so the prefix names ours exactly. A before/after diff
would have been the wrong shape: the operator's other agents are recorded there *while the gate
runs*, and a guard that flags their work is a guard that gets switched off.

If the sandbox cannot be created, the gate **refuses and runs nothing**. Falling back to the real
bench root is the defect; it is not an acceptable fallback.
