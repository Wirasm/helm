...
/// Which clipboard a program's write lands on (#297).
///
/// The selection clipboard is advertised (`supports_selection_clipboard`, as Ghostty.app
/// does) so that `copy-on-select` writes there and not to the one pasteboard the operator
/// has; otherwise every drag would replace it. Nothing exposes a selection or primary
/// clipboard here, so a write aimed at either is dropped, never redirected to the system
/// one: that redirect let `OSC 52` at the selection target replace the operator's clipboard.
/// A comparison, not a switch: `ghostty.h` names two constants, but ghostty passes its own
/// `primary = 2` across the boundary as a raw value no constant names.
enum TerminalClipboardWrite {
    static func reachesPasteboard(_ clipboard: ghostty_clipboard_e) -> Bool {
...
private enum TerminalCallbacks {
    ...
    static func writeClipboard(
        ...
        guard TerminalClipboardWrite.reachesPasteboard(clipboard) else { return }
