...
final class WorkbenchPlacementTests: XCTestCase {
    ...
    func testASpawnWhileACanvasHasFocusIsNotATabOnTheCanvas() {
        ...
        XCTAssertEqual(
            placement, .column,
            "it gets a column of its own at the right end, never a row beside the canvas")
    }
    ...
    /// Every spawn appends a column on the right, even when a column of terminals already
    /// exists, so the axis the operator was working in is never shortened.
    func testASpawnAlwaysGetsANewColumnEvenBesideAColumnOfTerminals() {
        ...
        XCTAssertEqual(
            bench.placementForSpawnedTerminal(), .column,
            "agents do not go where agents already are any more — every spawn appends a "
                + "column on the right, preserving the vertical space the operator is using")
    }
