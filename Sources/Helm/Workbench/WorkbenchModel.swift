...
final class WorkbenchModel: ObservableObject {
    ...
    ///
    /// Where it lands is `Workbench.placementForSpawnedTerminal()`'s — a new column at the
    /// right end, decided by the bench rather than by whatever the operator last clicked. That
    /// it *appears* rather than seizing is `Workbench.offer(_:at:)`'s (#125): `selected` and
