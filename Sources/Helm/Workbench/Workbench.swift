...
struct Workbench: Codable, Equatable {
    ...
    /// A spool spawn is a column at the right end (#177), so that is now something that
    /// happens repeatedly and unbidden, and shrinking the columns somebody is working in is
    /// its own kind of seizing.
    ///
    ...
    ///
    /// **A terminal pane only**: nothing else can hold an agent, and a canvas quietly growing
    /// one would be a value nothing can act on.
    ///
